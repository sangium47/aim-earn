    ...
    },
    {
      ...
      render: (item) => (
        <div onClick={(e) => e.stopPropagation()}>
          <Switch
            checked={!!publishMap[item.sku]}
            onChange={(next) =>
              setPublishMap((prev) => ({ ...prev, [item.sku]: next }))
            }
            ariaLabel={`Toggle publish for ${item.name}`}
          />
        </div>
      ),
    },
  ...
  return (
    <main className="flex flex-col gap-3 md:gap-6 p-3 md:p-6 lg:p-8">
      ...
      <Table
        ...
        onRowClick={(item) =>
          router.push(`/distributor/products/${encodeURIComponent(item.sku)}`)
        }
      />
    </main>
  );
