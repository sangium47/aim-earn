  ...
  return (
    <main className="flex flex-col gap-3 md:gap-6 p-3 md:p-6 lg:p-8">
        ...
        actions={
          <>
            <ProductActions
              ...
              disabledDownload
            />
            <div className="flex gap-1 bg-white px-2 py-1 rounded-lg items-center border border-brand-border h-[36px]">
              <Switch
                checked={!!publishMap}
                onChange={() => setPublishMap(!publishMap)}
                ariaLabel={`Toggle publish for ${id}`}
              />
              <p className="text-xs md:text-sm">Publish</p>
            </div>
          </>
        }
      />
      <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-[0_3px_7.6px_0_rgba(12,12,13,0.03)]">
        ...
        <CommissionSection commissions={commissions} promotions={promotions} />
      </div>
    </main>
  );
}
