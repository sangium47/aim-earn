...
import { AFFILIATES, PROFILE_TAB_ITEMS } from "@/components/mock";
...
export default function AffiliateMemberDetailPage() {
  const router = useRouter();
  const params = useParams() as { id?: string } | null;
  const rawId = decodeURIComponent(params?.id ?? "");

  const match = useMemo(() => AFFILIATES.find((a) => a.id === rawId), [rawId]);
  const notFound = rawId !== "" && !match;

  useEffect(() => {
    if (notFound) router.replace("/admin/affiliate/affiliates");
  }, [notFound, router]);
  ...
  return (
    <main
      className={`flex flex-col gap-4 md:gap-10 p-3 md:p-6 lg:p-8${tab === "profile" ? " pb-36" : ""}`}
    >
